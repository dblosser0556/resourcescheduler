  ...
  <table class="wp-list-table widefat fixed striped posts">
    <thead>
      <tr>
        ...
        <th class="manage-column column-title column-primary"><?= __('Start', 'resourcescheduler'); ?></th>
        <th class="manage-column column-title column-primary"><?= __('End', 'resourcescheduler'); ?></th>
    ...
    <tbody>
      <?php for ($i = 0; $i < sizeof($reservations); $i++) {
        ...
        <tr>
          ...
          <td><?= $item->type ?></td>
          <td>
            <?= date_i18n(get_option('date_format'), strtotime($item->start)) ?>
          </td>
          <td>
            <?= date_i18n(get_option('date_format'), strtotime($item->end)) ?>
          </td>
          <td>
            <form method="POST">
